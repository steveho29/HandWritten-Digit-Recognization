...
def training(n_epochs, batch_size, model_path, is_continue_train=False):
    ...
    for epoch in range(n_epochs):
        ...
        print("Epoch {} - Training Loss: {} - Validation Accuracy: {}".format(epoch, running_loss / len(train_data),
                                                                     validation_accuracy))
...
def testing_fully_connected_model(model_path, batch_size):
    model = torch.load(model_path)
    test_data = get_data(batch_size=batch_size, is_training=False)
    correct, total = 0, 0
